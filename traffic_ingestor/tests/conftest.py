...
def mock_store_table_service_client():
    with patch("traffic_ingestor.helper_functions.store_event_in_table_module.TableServiceClient") as mock_service:
...
def mock_cleanup_table_service_client():
    with patch("traffic_ingestor.helper_functions.cleanup_inactive_events_module.TableServiceClient") as mock_service:
        mock_table_client = MagicMock()
        mock_service.from_connection_string.return_value.get_table_client.return_value = mock_table_client
        yield mock_table_client
